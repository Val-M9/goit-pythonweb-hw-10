...
from src.schemas.schemas import ContactResponse, ContactModel, ContactUpdate, BirthdaysResponse
...
async def read_contacts_with_upcoming_birthdays(
    ...
    contact_service = ContactService(db)
    contacts = await contact_service.get_contacts_with_upcoming_birthdays(days)
    if not contacts:
